...
CLUSTER_ID=$(curl -s --request GET   \
  --url "https://portal-se-emea.edbhcp.com/api/v1/projects/${PROJECT_ID}/clusters"  \
  --header "x-access-key: ${ACCESS_KEY}" \
  | jq -r ".data[] | select(.name == \"${CLUSTERNAME}\") | .clusterId")
...
curl -s \
  ...
  --header "x-access-key: ${ACCESS_KEY}" \
  --request DELETE \
  "https://portal-se-emea.edbhcp.com/api/v1/projects/${PROJECT_ID}/clusters/${CLUSTER_ID}" \
  | jq
